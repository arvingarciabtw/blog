...
description: "A blog article explaining how state and rendering works in React."
pubDate: "Oct 12 2025"
---
...
We need state because we want to retain data between renders. With local variables, this is not possible because they don't persist between renders. Also, changing these local variables don't trigger renders (as mentioned before, re-renders happen when the _state_ is updated).
...
This `useState` hook provides us two things: (1) a _state variable_ that holds our state, which retains data between renders and (2) a _state setter function_ that updates the state variable (which again, will trigger a re-render since the state is updated).
